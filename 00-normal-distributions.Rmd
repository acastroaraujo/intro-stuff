...
*The purpose of this notebook is to build some intuition behind normal distributions on how to work with them in the context of the Null Hypothesis Significance Testing framework (NHST).*
...
The main assumption behind the CLT is that the sample size is *big enough*.
...
The following graphs shows the sampling distribution (right panel) I constructed by taking 100,000 samples of size 40 from the original population (left panel).
...
p2 <- df |> 
  ...
  geom_vline(xintercept = mean(fake_data), color = "red") + 
  labs(title = "Sampling Distribution", caption = "built with 100,000 samples,\neach with a size of 40", y = "Frequency")


p1 + p2
```

In the real world we don't have access to the whole population. We also don't have access to the sampling distribution (i.e., we don't have enough money to sample the population more than once). However, *we can estimate the standard deviation of the sampling distribution which we call the* **standard error.**

## Null hypothesis testing

Because we don't know the mean of the population ($\mu$), we can instead compare our sample average to a different value which we'll call a *null hypothesis* ($H_0$).
...
But we're not there yet note that, we are still missing the parameter that tells us how dispersed (or concentrated) the distribution is. That is, we are missing the standard deviation $\sigma$.

If we want to add *that*, we need to *add* it in two places for it to still behave according to the rules of probability.
